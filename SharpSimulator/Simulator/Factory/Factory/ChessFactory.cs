...
namespace SharpSimulator.Factory {
	public class ChessFactory : AbstractMapFactory {
		public ChessFactory() {
		}
		...
		internal override List<AEntity> GenerateEntities()
		{
			...
			for (int i = 0; i < entitiesLength; i++)
			{
				...
				string color = MapFile.Map.entities[i].color;
				AEntity entityInstance = (AEntity)Activator.CreateInstance(Type.GetType(entityClassName), color);
